...
class DocuStreamTest {
    ...
    @Ignore("takes a long time to run each time..")
    @Test(timeout = 6000)
...
//    }
    ...
    fun f2_performanceTestInitialisation() {
        ...
        val vector = "RandomInitVector" // 16 bytes IV

        var cipher: DataCipher? = null

        for (i in 1..5000) {
            cipher = DataCipher(secretKey = key, ivParameter = vector)
        }
    ...
    fun f3_encryptAndDecryptProof() {
        ...
        val cipher = DataCipher("Bar12345Bar12345", "RandomInitVector")

        val encrypted = cipher.encrypt(testValue)
